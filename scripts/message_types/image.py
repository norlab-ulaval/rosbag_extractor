...
# def extract_rectified_images_from_rosbag(bag_file, topic_name, output_folder, image_ext="png"):
...
#                 if "bgr" in msg.encoding:
#                     np_image = cv2.cvtColor(np_image, cv2.COLOR_RGB2BGR)
#                 rectified_image = rectify_image(np_image
#                 cv2.imwrite(os.path.join(output_folder, f"{int(timestamp):d}.{image_ext}"), np_image)
    
#         print(f"Done ! Exported images to {output_folder}")
        
# def rectify_image(image, map1, map2):
